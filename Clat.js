...
class ClapatSlider {
  ...
  transformItems() {
    ...
    for (let i = 0; i < this.items.length; i++) {
      ...
      (n < 0 &&
        s.end + n < e + t + s.length - this.length &&
        (n += this.length),
  ...
  updateUI() {
    ...
    for (let r = 0; r < this.items.length; r++) {
      let o = this.items[r],
        h = n * l;
      (h < 0 &&
        o.end + h < e + t + o.length - this.length &&
        (h += this.length),
  ...
  snapTargetOnDrag(e) {
    ...
    for (let l = 0; l < this.items.length; l++) {
      let a = this.items[l],
        r = s * n;
      (r < 0 &&
        a.end + r < t + i + a.length - this.length &&
        (r += this.length),
  ...
  snapTargetOnWheel(e, t) {
    ...
    for (let e = 0; e < this.items.length; e++) {
      let t = this.items[e],
        r = n * l;
      (r < 0 &&
        t.end + r < i + s + t.length - this.length &&
        (r += this.length),
    ...
    for (let n = 0; n < this.items.length; n++) {
      let l = this.items[n],
        h = r * o;
      (h < 0 &&
        l.end + h < i + s + l.length - this.length &&
        (h += this.length),
  ...
  update() {
    ...
    for (let l = 0; l < this.items.length; l++) {
      let a = this.items[l],
        r = i * s;
      (r < 0 &&
        a.end + r < e + t + a.length - this.length &&
        (r += this.length),
        r > 0 && a.end + r > this.length && (r -= this.length));
  ...
  onPrev(e) {
    ...
    for (let e = 0; e < this.items.length; e++) {
      let l = this.items[e],
        a = s * n;
      (a < 0 &&
        l.end + a < t + i + l.length - this.length &&
        (a += this.length),
